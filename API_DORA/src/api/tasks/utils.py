...
# вход: текст
def rag_run(query: str, history: str, vectorstore=Chroma(persist_directory="./chroma",
                                                         embedding_function=OpenAIEmbeddings(
                                                             openai_api_key=OPENAI_TOKEN))) -> dict:
    ...
    if not conversation:
        ...
        for fragment in conversation.split(';'):
            if fragment.startswith('Пользователь'):
                chat_history.append(HumanMessage(content=fragment))
            elif fragment.startswith('БОТ'):
                chat_history.append(AIMessage(content=fragment))

    response = agent_executor.invoke(
        {"input": query, "chat_history": chat_history})['output']
